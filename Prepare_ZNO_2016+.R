...
# helpers
{
  ...
  rename_ukr <- function(data) {
    ...
    data <- data %>%
      rename_with(
        .cols = matches("^ukr"), .fn = ~ str_replace(.x, "ukr", "uml")
      )
  ...
  remove_extra_reg_data <- function(data) {
    ...
    data <- data %>%
      mutate(across(
        ends_with(columns), ~ gsub(., pattern = patterns, replacement = "")
      )) %>%
  ...
  align_teststatus <- function(data) {
    ...
    for (i in 1:length(subjects)) { # nolint: seq_linter.
      prefix <- subjects[i]
  ...
  assign_datatypes <- function(data) {
    ...
    data <- data %>%
      mutate(
        across(ends_with("ball100"), as.numeric),
        across(c(matches("ball12$|ball$|birth")), as.integer),
        across(c(matches("name$|test$|lang$|status$")), as.factor),
        across(c(
          matches("ptname$|ptregname$|ptareaname$|
            pttername$|eoname|KOATUU_|ATU_")
        ), as.character)
      )
  ...
  assert_raw_data <- function(data, year) {
    ...
    required_string <- "year|outid|birth|^sextypename|
      ^regname|^areaname|^hromname|^tername|
    ...
    if ("classprofilename" %in% colnames(data)) {
      assert_all_are_equal_to(
        sum(is.na(data$classprofilename)),
        sum(is.na(data$eoname))
      )
    }
    ...
    for (i in 1:length(names(ball100))) { # nolint: seq_linter.
      assert_all_are_in_range(
        ball100[[i]], lower = 0, upper = 200, na_ignore = TRUE
      )
    }
    ...
    if (length(ball12 > 0)) {
      for (i in 1:length(names(ball12))) { # nolint: seq_linter.
        assert_all_are_in_range(
          ball12[[i]], lower = 1, upper = 12, na_ignore = TRUE
        )
      }
    }
    ...
    if (length(ball) > 0) {
      for (i in 1:length(names(ball))) { # nolint: seq_linter.
        assert_all_are_in_range(
          ball[[i]], lower = 0, upper = 120, na_ignore = TRUE
        )
      }
  ...
  assert_schools_data <- function(data, yr) {
    ...
    # assert NA for required fields
    required_string <- "^eoregname|^eoareaname|^eohromname|
      ^eotername|ATU_first|ATU_second|ATU_third|ATU_fourth"
  ...
  compare_columns <- function(base, comp) {
    ...
    if (!res) {
      mism <- compare_df_cols(base, comp,
        return = "mismatch",
        bind_method = "rbind"
      )
      ...
      if (s_name %in% names(data)) {
        data <- data %>%
          mutate(
            {{ reg_name }} := if_else(
              is.na(get(s_name)) &
                !is.na(get(reg_name)), NA_character_, get(reg_name)
            ),
            {{ area_name }} := if_else(
              is.na(get(s_name)) &
                !is.na(get(area_name)), NA_character_, get(area_name)
            ),
            {{ ter_name }} := if_else(
              is.na(get(s_name)) &
                !is.na(get(ter_name)), NA_character_, get(ter_name)
            )
          )
      }
...
# functions
{
  ...
  add_tertypename <- function(data, KOATUU_path, KOATUU_ref) {
    ...
    fourth <- data %>%
      ...
      mutate(
        tertypename =
          case_when(
            str_detect(category, villages_categories) ~ "село",
  ...
  add_hromname <- function(data, ter_list) {
    ...
    data <- bind_rows(
      data[hrom, on = c(setNames(
        "ATU_third",
        paste0(ter_list$col_pref, "_", "ATU_third")
      )), nomatch = 0],
      data[ATU$adm, on = c(setNames(
        "ATU_first",
        paste0(ter_list$col_pref, "_", "ATU_third")
      )), nomatch = 0]
    ) %>%
  ...
  add_ATU_new <- function(data, ter_list) {
    ...
    # ATU first
    obl <- bind_rows(
      data[ATU$oblast, on = c(setNames("name", ter_list$reg)), nomatch = 0],
    ...
    rajon <- bind_rows(
      obl[
        ...
        on = c(
          ATU_first = "ATU_first",
          setNames("name", ter_list$area)
        ),
        nomatch = 0
      ],
      obl[
        get(ter_list$area) %in% c(ATU$adm$name),
      ][
        , ATU_second := ATU_first
      ],
      obl[!is.na(ter_list$hrom)][
        hr_dict,
        on = c(
          setNames("oblast", ter_list$reg),
          setNames("adm_center", ter_list$area)
        ),
        nomatch = 0
      ][
        ATU$rajon,
        on = c(ATU_first = "ATU_first", rajon = "name"),
        nomatch = 0
      ]
    ) %>%
    ...
    hrom_dist <- rajon[ATU$district,
      on = c(
        ATU_first = "ATU_first", ATU_second = "ATU_second",
        setNames("name", ter_list$ter)
      ), nomatch = 0
    ]
    ...
    fourth <- bind_rows(
      hromada[
        is.na(ATU_fourth) & tertypename == "місто",
      ][
        ATU$misto,
        on = c(ATU_third = "ATU_third", setNames("name", ter_list$ter)),
        nomatch = 0
      ],
      hromada[
        is.na(ATU_fourth) & tertypename == "селище міського типу",
      ][
        ATU$smt,
        on = c(ATU_third = "ATU_third", setNames("name", ter_list$ter)),
        nomatch = 0
      ...
      hromada[
        is.na(ATU_fourth) & tertypename %in% c("селище, село", "село"),
      ][
        ATU$village,
        on = c(ATU_third = "ATU_third", setNames("name", ter_list$ter)),
        nomatch = 0
    ...
    final <- bind_rows(f_na, f_s, f_smt, fourth) %>%
      rename_with(
        ., .cols = starts_with("ATU"),
        .fn = ~ str_c(ter_list$col_pref, "_", .x)
      )
  ...
  write_to_file <- function(data, path, year) {
    ...
    if (file.exists(path)) {
      ...
      if (!any(rw$year == year)) {
        ...
        print(str_c("data for ", year, " alredy exist"))
      }
    } else {
      fwrite(
        data, path, sep = ";", quote = FALSE,
  ...
  bind_school_id <- function(schools, tests, y) {
    ...
    school_unique_geo <- schools %>%
      group_by(region, area, town) %>%
      mutate(n = n()) %>%
      filter(n == 1) %>%
      ungroup()

    # find schools which are unique within towns from test dataset
    test_unique_geo <- tests %>%
      group_by(eoregname, eoareaname, eotername) %>%
      mutate(n = n()) %>%
      filter(n == 1) %>%
      ungroup()
    ...
    name_dup <- unique_name_test_data %>%
      group_by(eoname) %>%
      filter(n() > 1) %>%
      summarise(n = n())
...
# prepare raw dataset 2016+
{
  ...
  read_data(path_2022) %>%
    ...
    rename(
      ukrteststatus = teststatus, ukrtest = block1,
      ukrball100 = block1ball100, ukrball = block1ball,
      histtest = block2, histball100 = block2ball100,
      histball = block2ball, mathtest = block3,
      mathball100 = block3ball100, mathball = block3ball,
      ukrptregname = ptregname, ukrptareaname = ptareaname,
      ukrpttername = pttername
    ) %>%
  ...
  full <- read_data(path_2023) %>%
    select(-c(test, testdate)) %>%
    rename(
      ukrteststatus = ukrblockstatus, ukrtest = ukrblock,
      ukrball100 = ukrblockball100, ukrball = ukrblockball,
      histteststatus = histblockstatus, histtest = histblock,
      histball100 = histblockball100, histball = histblockball,
      mathteststatus = mathblockstatus, mathtest = mathblock,
      mathball100 = mathblockball100, mathball = mathblockball,
      physteststatus = physblockstatus, phystest = physblock,
      physball100 = physblockball100, physball = physblockball,
      chemteststatus = chemblockstatus, chemtest = chemblock,
      chemball100 = chemblockball100, chemball = chemblockball,
      bioteststatus = bioblockstatus, biotest = bioblock,
      bioball100 = bioblockball100, bioball = bioblockball,
      engteststatus = engblockstatus, engtest = engblock,
      engball100 = engblockball100, engball = engblockball,
      frateststatus = frablockstatus, fratest = frablock,
      fraball100 = frablockball100, fraball = frablockball,
      deuteststatus = deublockstatus, deutest = deublock,
      deuball100 = deublockball100, deuball = deublockball,
      spateststatus = spablockstatus, spatest = spablock,
      spaball100 = spablockball100, spaball = spablockball,
      ukrptregname = ptregname, ukrptareaname = ptareaname,
      ukrpttername = pttername
    )

  data_outside_ukr <- full %>%
    filter(regname == "Інші країни") %>%
    remove_useless_data(.) %>%
    align_teststatus(.) %>%
    align_tertypename(.) %>%
    align_regtypename(.) %>%
...
{
  insts <- fread(
    institutions_dictionary, encoding = "UTF-8",
    na.strings = c("", NA)
  ) %>%
    mutate(
      across(
        c(
          inst_name, old_name0, old_name1, old_name2, old_name3, old_name4),
        ~ str_replace_all(.x, c("„|”" = "", "`" = "'")
        ) %>%
    ...
    select(-c(
      closed_date, type, long, lat, parent, old_name0:old_name4,
      status, property, closed, edrpou
    ))


  # read dataset by schools
  school_dataset <- fread(
    path_zno_schools,
    colClasses = c("character"),
    encoding = "UTF-8"
  ) %>%
    mutate(across(
      eoname, ~ str_replace_all(
        .x, c("„|”" = "", "`" = "'")
      ) %>%
        gsub(pattern = "I", replacement = "І", ignore.case = TRUE)
    )) %>%
  ...
  keys <- list(
    key1 = c("school_ATU_fourth" = "ATU_fourth", "s_name"),
  ...
  for (i in 1:length(keys)) { # nolint: seq_linter.
  ...
  school_ds_year_x <- school_ds_year[
    , n := n_distinct(s_name), by = school_ATU_fourth
  ][n == 1, -"s_name"]

  insts_x <- insts[, n := .N, by = .(ATU_fourth)][n == 1, -"s_name"]

  data_u <- school_ds_year_x[
    insts_x, on = c("school_ATU_fourth" = "ATU_fourth"),
    nomatch = 0
  ]
  ...
  while (nrow(bind_rows(data, data_d)) < nrow(school_ds_year_init)) {
    d <- ds[
      ins,
      on = c("school_ATU_fourth" = "ATU_fourth"), nomatch = 0,
    ...
    ][, .SD[which.min(distance)], by = .(school_ATU_fourth)]
  ...
  data <- data %>%
    relocate(full_name, .after = eoname) %>%
    select(year, EDEBO_id, eoname:range_200)
